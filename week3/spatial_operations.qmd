...
# subset nz heights to just the features that intersect Canterbury
c_height1 <- nz_height[canterbury, ] # 70 highest elevation points within Canterbury
...
# check wheter or not the points overlap
...
# now each point represents a single values and no points are overlayed.
```
...
regions2 <- aggregate(x = us_states[, "total_pop_15"], # data to be agrregated
