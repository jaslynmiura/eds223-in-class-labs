...
landsat34 <- c(landssat3, landsat4)
...
# verify that the output has few values than original
...
# selecting certain parameters that meet our conditions and then setting other values as NA
...
summary(reclassified) # tells us the number of cells or pixels that are in each group 
...
# apply the NDVI function to Landsat bands 3 & 4
ndvi_rast <- lapp(landsat[[c(4, 3)]], fun = ndvi_fun) # nir is the 4th layer, red is the 3rd layer
...
elev_2 <- extend(zion_elevation, c(1, 200)) # adds one row of empty cells to the top and bottom of the raster and 200 columns of empty cells to the right and left of the raster
