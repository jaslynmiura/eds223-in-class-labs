...
tm_shape(world_elevation) + # plotting global elevations
  tm_raster() + # use tm_raster() for raster data
  tm_shape(world_vector) +
  tm_borders() +
  tm_shape(world_cities) + 
  tm_dots() + 
  tm_text("name", size = 0.7, col = "purple") # including text on top of the dots, using the column "name"
...
# fill the polygon with fixed color
tm_shape(world_vector) +
  tm_fill(fill = "lightpink")
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(values = c("gold", "violet")),
              fill.legend = tm_legend(title = "Life Expectancy (years)", 
                                      position = tm_pos_out("left"))) # tm_pos_out() says that we want the legend on the outside
...
tm_shape(world_vector) +
  tm_polygons(# fill = "wb_income_region",
              ...
              fill.legend = tm_legend("Regional Income")) +
  tm_title(text = "Global income",
           frame = TRUE) +
  tm_layout(text.fontfamily = "serif",
            style = "beaver",
            bg.color = "grey95") + 
  tm_style(style = "beaver",
         bg.color = "steelblue",
         frame = TRUE,
         frame.double_line = TRUE,
         color_vision_deficiency_sim = ("tritan")) +
  tm_crs("+proj=eqearth") # updating the coordinate reference system
