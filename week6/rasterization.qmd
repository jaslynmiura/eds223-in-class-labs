...
if(crs(elevation) == crs(boundary)){
  ...
  warning("updating coordinate systesm to match")
...
# update park boundary object name to clarify it's a polygon
park_polygon <- boundary

# create line version of park boundary
park_border <- boundary %>% 
  st_cast(. , "MULTILINESTRING") # converts on geometry to another
...
map1 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(3, 2, 1), multivariate = TRUE)) + # true color image
  tm_title(text = "true color")

map2 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(4, 3, 2), multivariate = TRUE)) + # near infrared to show plants as red
  tm_title(text = "NIR, red, green")

map3 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(5, 4, 2), multivariate = TRUE)) + # water appears as a darker blue/black
...
# define NDWI as the normalized difference
