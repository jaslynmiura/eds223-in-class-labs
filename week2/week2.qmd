...
world_df <- world %>% 
  select(-geom) # this doesn't actually remove the geom column
...
# the geom column is gone!
colnames(world_df)

class(world_df) # no longer an sf object

ncol(world_df) # now only have 10 columns instead of 11
...
# they both use different crs so they do not match
...
# the projection that we pick makes our map look different
...
world %>% 
  select(name_long, pop) # since geom is sticky, it remains even if we don't select it
```
...
world %>% 
  select(name = name_long,
         population = pop) # only selecting the name and population columns AND renaming them
```
...
n_countries <- world %>% 
  ...
  summarize(population = sum(pop, na.rm = TRUE),
            ...
            n_countries = n()) %>% 
  st_drop_geometry() # dropping the geomerty to speed up the code
...
world_pop_den <- world %>% 
  ...
  summarize(population = sum(pop, na.rm = TRUE),
            area_km2 = sum(area_km2, na.rm = TRUE),
            pop_den = sum(population/area_km2),
            n_countries = n()) %>% 
  st_drop_geometry() %>% 
